...
export default defineComponent({
  ...
  setup() {
    ...
    const sendRequest = async () => {
      try {
        const resp = await ky.post('http://127.0.0.1:8000/generate-image', {
          json: {
            image_style: selectedStyle.value,
            user_prompt: prompt.value
          },
          headers: {
            'Content-Type': 'application/json'
          }
        }).json<{ message: string, generated_image_id: string }>()

        generatedIdSession.value = resp.generated_image_id
      } catch (e) {
        console.error(e)
      }
      ...
      try {
        ...
        if (selectedFile.value != null) {
          ...
          await ky.post(`http://127.0.0.1:8000/generate-image/upload-image/${generatedIdSession.value}`, {
            body: form
          })
        } else {
          await ky.post(`http://127.0.0.1:8000/generate-image/upload-image/${generatedIdSession.value}`)

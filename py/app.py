...
def generate(prompt_text: PostUserPrompt):
    ...
    def resample(input, size, align_corners=True):
        ...
        if dh < h:
            kernel_h = lanczos(
                ramp(dh / h, 2), 2).to(input.device, input.dtype)
        ...
        if dw < w:
            kernel_w = lanczos(
                ramp(dw / w, 2), 2).to(input.device, input.dtype)
    ...
    class MakeCutouts(nn.Module):
        def __init__(self, cut_size, cutn, cut_pow=1.):
            ...
            self.augs = nn.Sequential(
                ...
                K.RandomSharpness(0.2, p=0.3),
                K.RandomAffine(degrees=30, translate=0.1,
                               p=0.8, padding_mode='border'),
                K.RandomPerspective(0.2, p=0.4),
                K.ColorJitter(hue=0.01, saturation=0.01, p=0.6))
            self.noise_fac = 0.1
    ...
    def resize_image(image, out_size):
        ...
        size = round((area * ratio)**0.3), round((area / ratio)**0.3)
